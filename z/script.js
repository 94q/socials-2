...
function addStuff(){at("main").innerHTML=`<h1 name="top"    id = "status_text" class="header animated"><a href="index.html">oh</a></h1><p  name="bottom" id = "text2" class="textiboi animated"></p>`;handleAnimations();}
...
at("status_text").innerHTML="[click to enter]";at("status_text").classList.add("begin");},100);});var temp=0
...
return;setTimeout(function(){console.log("animations over");var doc=document.getElementById("main");var inner=doc.innerHTML;doc.innerHTML+=`<div class="textiboi">
        ...
        <a class="textiboi animated fadeIn"  href="https://www.tiktok.com/@_3v3"><img src="z/images/tiktok.png" width="50px"></a>
        <a class="textiboi animated fadeIn"  href="https://instagram.com/andrxi4k"><img src="z/images/insta.png" width="50px"></a>
        
        `;console.log("running typewrite");indefiniteWrite();},550);}
function indefiniteWrite(){let strs=["~oh daddy!","oh vibes!","oh chill!","~oh icebear!","~oh daddy icebear"];var num=getRandomInt(strs.length);console.log("writing "+strs[num]);var typElement=document.getElementById('typElement');var typewriter=new Typewriter(typElement,{loop:true});typewriter.typeString(strs[num]).pauseFor(5000).deleteAll().pauseFor(100).callFunction(()=>{typewriter.stop();indefiniteWrite();}).start();}